...
const CaseComponent = ({ caseDetails }) => {
    ...
    return (
        <div className="bg-black border border-white/30 p-6 rounded-xl shadow-md hover:shadow-white/40 transition-all duration-350 ease-in-out relative mx-auto">
            <h2 className="text-sm text-left sm:text-2xl font-semibold mb-4 sm:text-center text-white">
                Case Details
            </h2>
            ...
            <div className="flex justify-between items-center mb-4">
                <div>
                    ...
                    <p className="text-sm font-semibold text-white">
                        {caseDetails.userName || "Unknown User"}
                    </p>
                </div>
            ...
            <div className="flex flex-col flex-wrap gap-4 mb-6 text-white text-sm">
                <div className="flex flex-col justify-between">
                    <div>
                        <strong className="text-gray-300">Case ID:</strong> {caseDetails._id}
                    </div>
                    <div>
                        <strong className="text-gray-300">Category:</strong> {caseDetails.category || "N/A"}
                    </div>
                </div>
                <div className="flex justify-between">
                    <div>
                        <strong className="text-gray-300">Date Started:</strong>{" "}
                        {caseDetails.dateStarted ? new Date(caseDetails.dateStarted).toLocaleDateString() : "N/A"}
                    </div>
                    <div>
                        <strong className="text-gray-300">Status:</strong>
                        <span
                            className={`inline-block ml-2 px-2 py-1 rounded-full font-semibold
            ...
            <div>
                ...
                <div className="relative flex -mx-5 items-center overflow-hidden">
                    <button
                        className="absolute sm:block hidden cursor-pointer left-0 top-1/2 transform -translate-y-1/2 bg-white/10 hover:bg-white/20 p-2 rounded-full z-10 ml-2"
                        onClick={() => handleScroll("left")}
                    >
                        <ChevronLeft className="text-white" />
                    </button>
                    <button
                        className="absolute  sm:block hidden cursor-pointer right-0 top-1/2 transform -translate-y-1/2 bg-white/10 hover:bg-white/20 p-2 rounded-full z-10 mr-2"
                    ...
                    <div
                        ref={scrollRef}
                        className="sm:flex hidden items-center min-h-[30vh] mx-15 overflow-x-auto gap-0 py-6 scroll-smooth custom-scrollbar"
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 border-4 transition-colors duration-300
                        ${isReached ? "bg-green-400 border-green-400" : "bg-gray-700 border-gray-500"}`}
                                        >
                                            <span className={`${isReached ? "text-green-900" : "text-gray-300"} font-bold`}>
                                                {index + 1}
                                            </span>
                                        </div>
                                        <p
                                            className={`${isReached ? "text-green-200" : "text-gray-400"
                                                } text-sm font-medium text-center`}
                                        >
                                            {stage.label}
                                        </p>
                                        {isCenter && (
                                            <p className="text-xs text-gray-300 text-center mt-1">{stage.desc}</p>
                                        )}
                                    </div>

                                    {!isLast && (
                                        <div
                                            className={`h-[2px] w-40 mx-2 rounded-full transition-colors duration-300 ${index < progressIndex ? "bg-green-400" : "bg-gray-600"
                                                }`}
                                        />
                                    )}
                                </div>
                    ...
                    {/* Mobile Progress UI */}
                    <div className="block sm:hidden mt-6 mx-auto mb-12 space-y-4">
                        {stageLabels.map((stage, index) => {
                            const isReached = index <= progressIndex;
                            const showSet = index === nextStageIndex;

                            return (
                                <div
                                    key={stage.label}
                                    className={`p-3 rounded-lg border ${isReached ? "border-green-500 bg-green-100/10" : "border-gray-500 bg-white/5"
                                        }`}
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        <p className={`font-semibold ${isReached ? "text-green-200" : "text-gray-300"}`}>
                                            {index + 1}. {stage.label}
                                        </p>
                                        {showSet && session.user.role !== "user" && (
                                            <button
                                                className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded-full shadow-sm"
                                                onClick={() => UpdateCaseProgress(index)}
                                            >
                                                Set
                                            </button>
