...
export default function UserUploadComponent({ caseDetails, onFulfilled }) {
  const [requests, setRequests] = useState([]);
  const [files, setFiles]       = useState({}); 

  useEffect(() => {
    fetch(`/api/case/${caseDetails._id}/requests`)
      .then(r => r.json())
      .then(setRequests);
  }, [caseDetails._id]);
  ...
  const upload = async (name) => {
    ...
    const res = await fetch(
      `/api/case/${caseDetails._id}/requests/${encodeURIComponent(name)}/upload`,
  ...
  return (
    <div className="flex flex-col gap-4 p-2">
      <h3 className="text-lg font-medium">Requested Documents</h3>
